...
describe('API Routes', () => {
  ...
  describe('GET /api/all', () => {
    ...
    it('should handle pagination parameters', async () => {
      ...
      await request(app).get('/api/all?page=2&limit=5').expect(200);
  ...
  describe('GET /api/book/:id', () => {
    it('should return a specific book by ID', async () => {
      ...
      const response = await request(app).get('/api/book/1').expect(200);
    ...
    it('should return 404 for non-existent book', async () => {
      ...
      const response = await request(app).get('/api/book/999').expect(404);

      expect(response.body).toHaveProperty('error', 'Book not found');
    });

    it('should validate book ID format', async () => {
      const response = await request(app).get('/api/book/invalid').expect(400);
